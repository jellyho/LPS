...
class MFDiT_REAL(nn.Module):
    ...
    def __call__(self, observations, actions, t, r=None, train=True, is_encoded=False):
        ...
        """
        # ==================================================================
        # 1. [Batch Promotion] Promote input dimensions
        # ==================================================================
        # If actions is 1D (Dim,), treat as unbatched.
        is_unbatched = (actions.ndim == 1)
        if is_unbatched:
            # (Dim,) -> (1, Dim)
            actions = jnp.expand_dims(actions, axis=0)
            
            # Expand t, r to (1,) or (1, 1)
            if t is not None: t = jnp.expand_dims(t, axis=0)
            if r is not None: r = jnp.expand_dims(r, axis=0)
            
            # Batch-expand observations (Dict or Array)
            # jax.tree_map expands all leaves (image, state, etc.) to (1, ...).
            observations = jax.tree_util.tree_map(
                lambda x: jnp.expand_dims(x, axis=0), 
                observations
            )
        
        # From this point, B always exists. (B=1 for unbatched inputs)
        B = actions.shape[0]

        embeddings = []

        # ==================================================================
        # 2. Embedding Logic (same as before but B is guaranteed)
        # ==================================================================
        ...
        if isinstance(observations, (dict, flax.core.FrozenDict)):
            if 'image' in observations and self.encoder is not None:
                ...
                if not is_encoded:
                    # Encoder (MultiViewWrapper also handles B=1 inputs correctly)
        ...
        # (C) Action Embedding
        # Safe to reshape regardless of B being 1 or N
        ...
        # ==================================================================
        # 4. [Batch Demotion] Restore dimensions
        # ==================================================================
        if is_unbatched:
...
# Common default configuration (modifiable)
COMMON_CONFIG = {
    'mlp_ratio': 4.0,
    'tanh_squash': False,
...
# -----------------------------------------------------------------------------
# 1. Nano & Tiny (optimized for real-time control, Raspberry Pi/Jetson, etc.)
# -----------------------------------------------------------------------------
# Very lightweight and fast. Good for preventing overfitting with small datasets.
...
MFDiT_Tiny = functools.partial(
    MFDiT_REAL,
    hidden_dim=384, # Same as DiT-Small but with shallower depth
    depth=8,
...
# -----------------------------------------------------------------------------
# 2. Small & Base (standard DiT/ViT configurations, recommended for ~4GB datasets)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 3. Large & XL (for large-scale data / complex tasks)
# -----------------------------------------------------------------------------
# DiT-L (Large)
